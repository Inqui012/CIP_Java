...
/**
 * Servlet implementation class Board_Contoller
 */
@WebServlet("/")
public class Board_Contoller extends HttpServlet {
	...
	public String getView(HttpServletRequest request) {
        ...
        try {
        	...
        	Board_DTO b = dao.getView(board_no);			
	...
	public String getViewForEdit(HttpServletRequest request) {
        int board_no = Integer.parseInt(request.getParameter("board_no"));
        try {
        	Board_DTO b = dao.getViewForEdit(board_no);			
			request.setAttribute("board", b);
		} catch (Exception e) {
			e.printStackTrace();
			ctx.log("게시글을 가져오는 과정에서 문제 발생!!");
