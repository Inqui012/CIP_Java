...
### Thread 와 Process
운영체제가 프로그램을 실행하고 있는 프로그램을 프로세스라고 한다.  
멀티태스킹은 여러개의 작업을 동시에 처리하는것이고 일반적으로 프로세스가 여러개가 생성된다.  
가끔 프로세스 하나에서 여러 작업을 할 수 있도록 만들어진 프로그램도 있음. (문서 작성중 자동저장, 문법검사 같은)  
스레드는 프로세스 안에서 실제로 기능을 수행하는 주체. 프로세스 안에는 최소 하나의 스레드가 존재한다.  
운영체제 > 프로그램(프로세스) > 스레드  
  
멀티 프로세싱, 멀티 스레딩의 방법에는 병렬, 병행 처리방법이 있는다.
* 병렬 = 2개 이상의 프로세서(cpu 코어)가 각각 다른 프로세스를 실행하여 동시에 작업한다.
* 병행 = 하나의 프로세스(cpu 코어)가 여러 프로세스를 재빠르게 돌아가며 조금씩 실행한다. = context switching.
프로그램을 효율적으로 만들었다면 병렬과 병행을 동시에 사용할 수도 있다.  
  
멀티 프로세스의 경우 오류가 생겨 프로세스 하나가 사라지더라도 상위 주체인 프로그램의 실행에는 문제가 없다.  
반면 스레드의 경우 하나의 스레드에 오류가 날 경우 상위 주체인 프로세스가 종료되어버린다.

각각의 프로세스는 메모리를 분리해서 각각 가지고 있고, 스레드는 프로세스가 가지고 있는 메모리를 공유하여 사용한다.  
스레드가 개별적으로 가지는것은 메모리의 Stack 영역 뿐이다.  

메인 스레드 = main() 메소드를 실행하면서 시작되는 가장 기본적인 스레드?  
만약 main() 를 실행하는 도중에 다른 코드를 같이 진행해야한다면 메인 스레드가 새로운 스레드를 생성하여 멀티스레딩을 진행 한다. 이 경우 모든 스레드의 작업이 종료될때까지 프로세스가 종료되지 않는다. 분기를 나누는 느낌...?  
자바의 경우 메인 이외의 스레드를 작성할경우 Runnable, Thread 를 이용하여 익명객체로 구현하는 일이 많다.  
...
### UML
소프트웨어 모델링에서 사용하는 언어. 자세한 내용은 study._07_modeling 참조.  
시스템의 복잡도에 따라서 사용하는 규모가 달라진다.  
유스케이스 다이어그램, 클래스 다이어그램 등이 여기에 속한다.  
  
* 캡슐화 (높은 응집도 / 낮은 결합도 권장)
> * 응집도 : 클래스 / 모듈의 요소들이 얼마나 밀접하게 관련되어 있는지.  
	하나의 클래스 / 모듈이 가지고 있는 필드나 메소드가 같은 목적을 위해 기능하는 것들인지?  
> * 결합도 : 기능을 실현하는데 있어서 다른 클래스 / 모듈 에 얼마나 의존적인지.  
	한곳의 코드를 수정할경우 얼마나 많은 다른 코드를 수정해야하는지?
...
```
ArrayList처럼 지정된 제네릭타입에 맞는 데이터를 여러개 가질 수 있는 클래스.  
특이점으로는 '마지막에 추가된 데이터가 먼저 나오는' 형식을 진다는 것이다. LIFO(Last In First Out).  
Queue 와는 반대되는 특성이다.  
  
Vector 클래스를 상속받는 관계라서 Vector 에서 사용하는 메소드도 전부 사용 가능한것 같네.
  
```java
var.push();
```
ArrayList 의 .add(); 와 동일하다. 스택의 마지막 인덱스 뒤에 지정한 데이터를 추가하는 메소드.  
  
```java
var.pop();
```
스택의 마지막 인덱스에 있는 데이터를 제거하는 메소드. 이것때문에 LIFO 라고 하는듯?  
  
```java
var.peek();
```
스택의 마지막 인덱스에 있는 데이터 값을 가져오는 메소드. 마지막 인덱스 번호를 몰라도 값은 볼 수 있겠다.  
  
```java
var.clear();
```
스택에 저장하고 있었던 모든 값을 null 값으로 되돌림. size() 가 0이 되도록한다.  
...
### 기타 메모
```java
Collections.frequency(콜렉션 프레임워크, 비교할 값);
```
콜렉션 프레임워크(LIST, MAP, SET)의 종류중에서 지정된 값이 얼마나 나오는지의 빈도수를 체크함. 반환하는 값은 숫자.  
기본적으로 값이 있다면 1이고, 없으면 0, 중복된 값이 존재한다면 2 이상이 나옴.  